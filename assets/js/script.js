const userInput = document.querySelector('#date');
const calculateBtn = document.querySelector('#calculate-btn')
const yearEle = document.querySelector('#year')
const monthEle = document.querySelector('#month')
const dayEle = document.querySelector('#day')
const message = document.querySelector('.message')

userInput.max = new Date().toISOString().split('T')[0]

const calculate = () => {
    if(userInput.value === '') {
        message.innerHTML = 'Please enter your birth date!'
        return
    }
    message.innerHTML = ''
    let birthDate = new Date(userInput.value);
    let today = new Date();

    let d1 = birthDate.getDate();
    let m1 = birthDate.getMonth() + 1;
    let y1 = birthDate.getFullYear();
    
    let d2 = today.getDate();
    let m2 = today.getMonth() + 1;
    let y2 = today.getFullYear(); 

    let d3, m3, y3;

    y3 = y2 - y1;

    if(m2 >= m1) {
        m3 = m2 - m1
    } else {
        y3--
        m3 = 12 + m2 - m1
    }

    if(d2 >= d1) {
        d3 = d2 - d1
    } else {
        m3--
        d3 = getDayInMonth(y1, m1) + d2 - d1
    }

    if(m3 < 0) {
        m3 = 11
        y3--
    }

    function getDayInMonth(year, month) {
        return new Date(year, month, 0).getDate();
    }

    yearEle.innerHTML = y3 + (y3 == 0 ? '' : y3 < 1 ? ' Year' : ' Years')
    monthEle.innerHTML = m3 + (m3 == 0 ? '' : m3 < 1 ? ' Month' : ' Months')
    dayEle.innerHTML = d3 + (d3 == 0 ? '' : d3 < 1 ? ' Day' : ' Days')
}

calculateBtn.addEventListener('click', calculate)